...
Канонічний реєстр операційного стану та control-контексту ForPrint: замовлення, взаємодії, робочі статуси, історія виконання та авторизований operational write boundary; фізична persistence topology лишається централізованою і не є власністю одного модуля.
...
Ти працюєш над модулем `forprint_operations_control_registry` у системі ForPrint. Твоя роль, межі відповідальності, вхідні й вихідні дані мають відповідати цьому guide. Якщо поточна реалізація модуля конфліктує з цим описом, не змінюй архітектуру самостійно — сформуй запит до ForPrint System Blueprint через `coordination/incoming_requests/forprint_operations_control_registry/new/`.
