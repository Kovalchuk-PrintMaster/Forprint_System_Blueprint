...
SECTION = "dependency_enrichment_v0_1"
...
def validate(
    *,
    ...
    load_yaml(capability_context_path)
    ...
    if expected_source_sha256 is not None and actual_source_sha256 != expected_source_sha256:
        errors.append("accepted REDM v0.3 SHA-256 drift")

    section = candidate.get(SECTION)

    if not isinstance(section, dict):
        errors.append("dependency enrichment section is missing")
        section = {}
    ...
    if base_candidate != source:
        errors.append(
            "candidate modifies accepted REDM content outside the additive enrichment section"
        )
    ...
    if not references_path(
        ...
        errors.append("source_snapshot does not reference REDM v0.3")

    if not references_path(
        section.get("capability_context_snapshot"),
        capability_context_path,
        repo_root=effective_root,
    ):
        errors.append("capability context does not reference RCI v0.4")

    evidence_sources = section.get("evidence_sources")

    if not isinstance(evidence_sources, list) or len(evidence_sources) < 4:
        errors.append("at least four dependency evidence sources are required")
    ...
    if unresolved.get("dependency_edges_not_inferred") is not True:
        errors.append("unverified dependency edges must not be inferred")

    policy = section.get("evidence_policy")

    if not isinstance(policy, dict):
        errors.append("dependency evidence policy is missing")
        policy = {}
    ...
    for key in (
        ...
        if policy.get(key) is not True:
            errors.append(f"dependency evidence policy flag must be true: {key}")

    entries = section.get("dependency_index_entries")

    if not isinstance(entries, list) or not entries:
        errors.append("dependency index entries are missing")

    if not references_path(
        record.get("candidate_redm"),
        candidate_path,
        repo_root=effective_root,
    ):
        errors.append("enrichment record candidate REDM path mismatch")

    if not references_path(
        record.get("source_redm"),
        source_path,
        repo_root=effective_root,
    ):
        errors.append("enrichment record source REDM path mismatch")

    if not references_path(
        record.get("capability_context_rci"),
        capability_context_path,
        repo_root=effective_root,
    ):
        errors.append("enrichment record capability context path mismatch")

    passed = not errors

    return {
        "schema_version": ("redm_dependency_enrichment_validation_report_v0_1"),
        "metadata": {
        ...
        "summary": {
            ...
            "dependency_index_entry_count": (len(entries) if isinstance(entries, list) else 0),
...
def main() -> int:
    ...
    parser.add_argument("--capability-context", required=True)
    ...
    report = validate(
        ...
        capability_context_path=Path(args.capability_context),
