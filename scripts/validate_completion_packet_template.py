...
REQUIRED_EXAMPLE_STANDARDS_REVIEWED: tuple[str, ...] = (
...
def validate_template(project_root: Path = PROJECT_ROOT) -> list[str]:
    ...
    standards_reviewed = packet.get("standards_reviewed")
    if isinstance(standards_reviewed, list):
        for required_standard in REQUIRED_EXAMPLE_STANDARDS_REVIEWED:
            if required_standard not in standards_reviewed:
                errors.append(
                    "completion_packet.example.yaml `standards_reviewed` "
                    f"must include `{required_standard}`"
                )
    ...
    for required_phrase in (
        ...
        "no_production_api",
        "timestamp",
        "modular_topology_and_resilience",
        "third_party_reuse",
    ):
